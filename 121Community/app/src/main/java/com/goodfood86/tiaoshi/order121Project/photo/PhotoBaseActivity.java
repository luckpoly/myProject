package com.goodfood86.tiaoshi.order121Project.photo;
...
public abstract class PhotoBaseActivity extends Activity implements EasyPermissions.PermissionCallbacks{
    ...
    protected void takePhotoAction() {
        if (!DeviceUtils.existSDCard()) {
            String errormsg = getString(cn.finalteam.galleryfinal.R.string.empty_sdcard);
            toast(errormsg);
            if (mTakePhotoAction) {
    ...
    private void takePhotoFailure() {
        String errormsg = getString(cn.finalteam.galleryfinal.R.string.take_photo_fail);
        if (mTakePhotoAction) {
            resultFailure(errormsg, true);
        } else {
            toast(errormsg);
    ...
    protected void resultData(ArrayList<PhotoInfo> photoList) {
        ...
        if (callback != null) {
            if ( photoList != null && photoList.size() > 0 ) {
                ...
                callback.onHanlderFailure(requestCode, getString(cn.finalteam.galleryfinal.R.string.photo_list_empty));
            }
        }
        finishGalleryFinalPage();
    }
