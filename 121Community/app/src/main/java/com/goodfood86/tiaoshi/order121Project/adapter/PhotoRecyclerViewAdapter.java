...
import com.goodfood86.tiaoshi.order121Project.activity.FabuActivity;
import com.goodfood86.tiaoshi.order121Project.activity.FrientFabuActivity;
...
public class PhotoRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
     ...
     * 一行最多显示的图片数量
     */
    public static final int NUM_ITEM = 9;
    ...
    public PhotoRecyclerViewAdapter(Context mContext, FragmentManager fragmentManager, List<PhotoInfo> mPhotoInfoList, XCallbackListener mXCallbackListener,String from) {
        this.mContext = mContext;
        this.fragmentManager=fragmentManager;
        this.mPhotoInfoList = mPhotoInfoList;
        mInflater = LayoutInflater.from(mContext);
...
//                ((FootViewHolder) holder).mImageAdd.setVisibility(View.VISIBLE);
            ((FootViewHolder) holder).mImageAdd.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (!TextUtils.isEmpty(from)&&from.equals("zixun")){
                        ((KuaisuZixunActivity) mContext).openChoiceDialog(mContext,fragmentManager);
                    }else if (!TextUtils.isEmpty(from)&&from.equals("friend")){
                        ((FrientFabuActivity) mContext).openChoiceDialog(mContext,fragmentManager);
                    }else {
                        ((FabuActivity) mContext).openChoiceDialog(mContext,fragmentManager);
                    }
                }
            });
//            }
